...
public function  CaseReportjss(){
	$cases =  CaseReport::all()   ; 
	//$cases    = CaseReport::select('id','nuberof_person_involve', 'name_person_involve', 'color','surname_person_involve','phone_person_involve' ,'email_person_involve' , 'description' , 'gps_lat','gps_lng' ,'img_url','category','sub_category','sub_sub_category')->get();
        ...
        if ($api_key) {
			...
			// $api_key      = UserNew::select('api_key')->where('api_key','=',$key)->first();
             ...
             if(sizeof($user) > 0)
             {
                ...
                ->join('users', 'cases.user', '=', 'users.id')
                ->where('cases.user','=',$user->id)
                ->select(\DB::raw("cases.id ,cases_statuses.name as status,cases.description,cases.location,cases.name_person_involve,cases.incident_date_time,cases.img_url,categories.name as category,`sub_categories`.name as sub_category,`sub_sub_categories`.name as sub_sub_category"))
                ->get();
